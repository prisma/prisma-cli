/**
 * DRAFT v4 — the unified CLI engine's public interface.
 * v1 initial · v2 round-1 fixes · v3 return-site presentation ·
 * v4 round-2 fixes + operator rulings: completed/errored semantics,
 * --format with --json alias, one log-level mechanism, prompt defaults
 * under --yes, "presentations" naming, outcome-code catalogue.
 * Prior versions preserved as -v1/-v2/-v3.ts; review artifacts in
 * ./reviews/.
 *
...
export interface PresentedResult<T> {
  ...
  readonly data: T
  /** The outcome code selected at the return site; must be a key of the
   *  definition's catalogue. Omitted = 0. */
...
export interface CommandContext<TConfig = undefined> {
   ...
   *  only constructor of PresentedResult. `outcomeCode` must be a key of
   *  the definition's catalogue (engine-verified). */
  readonly present: <T>(
    data: T,
    presentations: Presentations,
    opts?: { readonly outcomeCode?: number },
  ) => PresentedResult<T>
...
export declare const flag: {
  ...
  requiredString(spec: { brief: string; placeholder?: string; alias?: SingleChar }): FlagSpec<string>
  number(spec: {
    brief: string
    placeholder?: string
    alias?: SingleChar
  ...
  boolean(spec: { brief: string; alias?: SingleChar }): FlagSpec<boolean>
  ...
  repeated(spec: { brief: string; placeholder?: string; alias?: SingleChar }): FlagSpec<readonly string[]>
}

/** Single-character alias; longer strings are a construction error. */
...
> {
   ...
   * Rendered in help without executing anything; the return site selects
   * one via ctx.present's outcomeCode, which the engine verifies against
   * this catalogue. Absent = the command only exits 0/1/2/3.
   */
  readonly outcomeCodes?: Readonly<Record<number, string>>
...
export declare function defineCommand<
  ...
  TConfig = undefined,
>(
  def: Omit<CommandDefinition<TFlags, TPositionals, TConfig>, 'kind'>,
): CommandDefinition<TFlags, TPositionals, TConfig>
...
export type Block =
  ...
  /**
   * Structured errors carried inside a COMPLETED result (drift findings,
   * verification failures, config diagnostics). The engine renders each
   * with the same layout it uses for top-level errors (✖ summary (CODE),
   * Why, Fix) — consistent by construction; products never hand-build
   * error presentation. In the data/json side, carry the same errors as
   * their envelopes (`toEnvelope()`).
   */
