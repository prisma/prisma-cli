...
## Left open by the dev-build fix (2026-08-17) — waiting on the product repos

The CLI's release channel is blocked until both products publish a version it can use. This is deliberate: the conformance checks now refuse a release whose dependencies are dev builds (operator ruling 2026-08-17, `docs/oss/release-automation.md`), and nothing in this repo can lift it.

- **`@prisma/composer-cli@0.6.0` cannot be installed.** It was published with `npm publish` rather than `pnpm publish`, so its dependency on `@prisma/composer` is the literal string `workspace:0.6.0`; `npm install @prisma/composer-cli@0.6.0` fails with `EUNSUPPORTEDPROTOCOL`. Composer must publish a release with `pnpm publish`, from a commit that peers `@prisma/cli-engine` at the version prisma-cli ships (`0.6.0-dev.22` already does). Composer's repo has no check that installs its own tarball, which is why this reached the registry; prisma-cli's check 3 catches exactly this class and is worth porting there.
- **`@prisma/orm-toolchain` has no released version carrying the command family.** Its released `8.0.0-rc.1` has no engine relationship at all and its `./cli` export contains only `createContractEmitCommand`, `createFormatCommand`, `loadContractFromTs` — no `ormCommandFamily`. The engine-based family exists only in dev builds, and the newest (`8.0.0-rc.1-dev.46`) declares the engine at `0.0.9` rather than the version prisma-cli ships. prisma/prisma must adapt to the current engine (the work that was in the closed prisma#30034) and publish that as a release, declaring `@prisma/cli-engine` as an exact peer per ADR 0004.
- **The ORM's engine pin exception is the last one left.** `packages/cli/scripts/conformance.ts` carries one recorded exception, for the ORM depending on `@prisma/cli-engine@0.0.9` while the shell ships `0.1.1` — two engines in one install. Composer's exception is already gone. Delete the ORM's when it publishes the peer, and the engine-pin check can then move from equality to peer satisfaction.
- **Both product repos still need their publish-notification step** (the work in the closed composer#232 and prisma#30033): a `repository_dispatch` of type `product-published` to `prisma/prisma-cli`, placed immediately after the publish step and keyed on its outcome. The event type was renamed from `family-published`; `docs/oss/release-automation.md` carries the exact step. `DEPLOY_GITHUB_TOKEN` is provisioned in all three repositories (2026-08-17).
