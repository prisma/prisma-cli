...
- **The release checks did not catch a `prisma` bin that crashes on install.** "Check the release" passed on the rc.8 run: the conformance sandbox install evidently resolved the good `@prisma/orm-toolchain@8.0.0-rc.5` from `@prisma/cli`'s manifest by hoisting, masking the wrapper's stale rc.4 pin — a real `npm install prisma` hoists rc.4 instead and crashes. Two fixes, both worth doing: a check that the `prisma` and `@prisma/cli` manifests declare identical product pins (one-file script, catches the divergence class statically), and running the tarball check's bin-start against an install of the `prisma` package alone, so resolution happens the way a user's install resolves it.
- **Two manifests hand-carry the same pins.** `update-product-versions.mjs` rewrites both, but a hand-edit (this incident) can touch one. The static pin-equality check above is the guard; deriving one manifest from the other at pack time would remove the class entirely — design call.
