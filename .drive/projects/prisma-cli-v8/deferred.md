...
- **The management API has no direct deployment→service lookup.** `showDeployment` finds the owning service via `findAppForDeployment`, a scan of every project's service list and each service's deployments. Every id-targeted command pays that scan per run. An API endpoint answering "which app owns deployment X" would collapse it to one call.
