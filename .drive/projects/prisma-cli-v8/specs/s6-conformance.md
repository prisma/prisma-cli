# S6 — Conformance checker (slice contract, revision 3 — STOPs open)

Status: checks 1 and 2 BUILT and green against what this repo ships; check 3 and all CI wiring awaiting operator rulings on STOP-1 … STOP-9. Rev 3 folds both reviews (architect and principal-engineer, each accept-with-changes; all 35 findings adopted, dispositions in §7).
...
- Check 2's subjects reach the checker through a **relative source import of `packages/cli/src/v8/cli.ts`**, which is this repo's first cross-package relative source import. It is the only route: `@prisma/cli`'s `exports` map carries `./package.json` and nothing else, so the package cannot be imported by name, and the built v8 entry runs the CLI at top level rather than exporting anything. Importing it costs 546 ms and produces no output; its only top-level work is constructing the families. The dependency direction is `@repo/cli-conformance` → `@prisma/cli` and must stay that way, because the reverse would be a workspace cycle.
...
**STOP-1 — which repos are "both products", given prisma/prisma has no engine at all?** prisma/prisma contains zero occurrences of `@prisma/cli-engine` or `cli-engine` in any manifest, source file, doc or planning artifact, and zero occurrences of `defineConfigSection`, `ConfigSection` or `SectionValidation`. S5 has not started. All three checks would be no-ops in its publish path: no engine pin to compare, no `orm` section to hammer, and its own `check-publish-deps` already covers the tarball-manifest ground. Options: **(a) wire prisma-cli + composer now, and prisma/prisma as part of S5** — recommended; (b) wire all three now and accept the no-op in writing; (c) prisma-cli only. The project spec's FR7 names "prisma/prisma and composer" as the products while the slice brief named prisma-cli and prisma/prisma; the two readings disagree, which is why this is yours. Whichever you pick, see STOP-2's last paragraph on what happens to the DoD line.

**STOP-2 — build once in prisma-cli, or replace the checks the other two repos already have?** §2 is the fact: composer has five relevant checks and prisma/prisma six, all working, all tested, all wired. Composer already owns 3b's entire mechanism. Options: **(a) build the tool in prisma-cli where nothing exists, add only the genuinely missing cross-repo pin check to composer, and leave its existing checks alone** — recommended; (b) replace their scripts with the shared tool, a large diff in two repos S6 does not own, risking behaviour their checks have and the three-check mandate does not; (c) run the tool alongside, duplicating coverage.

Under (a), the DoD line "Conformance checker runs in CI for both products' publish paths" is **not met at S6's close**, and the honest move is to leave it unchecked with its dependencies recorded rather than reword it to describe what shipped. Revision 1's plan proposed rewording it; the architect review called that turning an unmet requirement into a met one by editing the requirement, and it is right. Confirm you want it left unchecked, or rule the rewording.

**STOP-3 — check 3c is red today and prisma-cli cannot make it green.** composer pins engine `0.0.9`; the shell ships `8.0.0-rc.1`, which is unpublished, so composer cannot pin it today either. Shipping 3c makes the publish path red on arrival. Options: **(a) ship 3c with one recorded, dated exception, keyed on the observed triple (family package, its engine pin, the shell's engine pin) so composer arriving at a third wrong version reopens the finding rather than staying suppressed** — recommended; (b) ship it failing and let it block publishing until composer moves; (c) warn-only, which is the option that quietly becomes permanent.

**STOP-4 — how does a second repo consume the tool without touching S7's release automation?** For composer to run it, it must be installable, which means published, which means a line in `publish.yml`'s publish list — and release automation is S7's. Options: **(a) if STOP-2 lands on (a), nothing needs publishing: prisma-cli runs the tool as a private workspace package and composer gets a small addition to its own `check-cli-engine-pin.mjs`** — recommended; (b) publish `@prisma/cli-conformance` in the existing lockstep and accept that S6 touched the publish list; (c) build it private now and defer cross-repo publication to S7, leaving the DoD unchecked at S6's close.

**STOP-5 — take the stronger form of 3c as well as the manifest comparison, and where does the sandbox live?** Two parts.

First: revision 1 asked you to confirm a proof that turned out to be wrong, and §3 records the correction. The install tree *can* carry the assertion, using a version-qualified override, and "exactly one engine copy resolves" catches a mismatch arriving through a family's own transitive dependency that a pairwise manifest comparison cannot see. It costs nothing extra given 3b installs anyway. **Recommended: take both** — the manifest comparison for its offline reach and its legible finding, the copy count for its strength.

Second: revision 2's reason for putting the sandbox outside the repo was also wrong, and §4 records that correction too. The choice is now **(a) an out-of-workspace `mkdtemp`, or (b) a gitignored in-repo path with `COREPACK_ENABLE_STRICT=0` set for the install** — recommended **(b)**, because a failed run leaves the sandbox on disk where it can be inspected, which is worth more than avoiding one environment variable. Either way the path carries the package name and is deleted at the *start* of a run rather than the end, so a failure leaves evidence instead of leaking a directory.

**STOP-6 — which bin does 3b start?** §3 records the finding that makes this matter: the only declared bin is the legacy commander shell, and it never loads composer, never resolves `/family`, and never touches the engine boundary. Options: **(a) start every entry in the packed `bin` map, which is self-maintaining and picks up `prisma` when S7 adds it, accepting that today it proves only that the retiring shell starts; (b) additionally start `dist/v8/cli.js` by path, which pins the v8 entry and the composer boundary now at the cost of naming a file the manifest does not** — revision 2 recommends **(b)**, reversing revision 1. Naming an unlisted file is a small, temporary ugliness; a check that runs for a whole slice without touching the boundary it was built for is worse.

**STOP-7 — a citation in the brief does not exist; confirm I substituted the right ruling.** The brief cites `specs/s5-orm.md` STOP-10 and requirement R-S5-28 as ruling the engine-mismatch class to be S6's. Neither exists: there is no `specs/s5-orm.md`, and `R-S5-28` and `STOP-10` appear nowhere in the repo. What *is* recorded is `deferred.md:37-51`, quoted in §1. I built 3c to that plus the brief's own description. The smallest of the questions; confirm and it closes.

**STOP-8 — who owns check 2, and should the engine export the section derivation?** Two parts, one question. First: R10 puts the never-throwing validator on the product that authors it, so a check living only in prisma-cli sees composer's validator only after composer published it. That is late, and it argues for the check running in each product's own suite with prisma-cli's run as a backstop. Second: §3 has the checker re-derive the engine's section union from `mountedCommands` and the families. That is the second list that drifts, which the plan elsewhere says it wants to avoid — the engine could export the derivation instead, but an engine change is forbidden in this slice without your ruling. Options: **(a) re-derive in the checker now, record the drift risk, and revisit when S5 adds the second real section** — recommended, it keeps the engine untouched; (b) rule the engine change in scope; (c) push check 2 into the products and drop it from prisma-cli.

**STOP-9 — does a registry outage block publishing?** 3b installs from the real registry, so a network failure, a registry outage, or a runner without npm makes the check unable to reach a verdict. Revision 2 said the install failure is "reported as its own finding kind carrying the installer's output", but a finding means a non-zero exit, which means an outage blocks the publish. Options: **(a) treat an install failure as a blocking finding — the publish path refuses to ship a tarball it could not verify** — recommended, because a publish is deliberate and rare and retrying is cheap; (b) treat it as a distinct skipped outcome that exits zero with a loud warning, which keeps publishing possible during an outage at the cost of a check that can be silently absent exactly when the tree is unusual. Measured cost either way: 31 s cold, 38 s with `--ignore-scripts`, ~438 packages.

## 6. Acceptance

Written against STOP-1(a), STOP-2(a) with the DoD line left unchecked, STOP-3(a), STOP-4(a), STOP-5 both forms, STOP-6(b), STOP-8(a). Rewritten if you rule otherwise.
...
- [ ] Check 3a/3b/3c run against both published packages: declared dependencies match packed output; the tarball installs out-of-workspace and every declared bin plus `dist/v8/cli.js` starts on plain Node at exit 0; pin agreement holds modulo §5's exception; exactly one engine copy resolves in the installed tree; the packed engine pin is an exact version; every mounted family package declares the engine.
...
- [ ] The DoD line stays unchecked with its S5 and composer dependencies recorded against it; `plan.md` §S6 records what S5 must wire in prisma/prisma.
