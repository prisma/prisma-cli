# S6 — Conformance checker (slice contract, revision 2 — STOPs open)

Status: DRAFT, awaiting operator rulings on STOP-1 … STOP-8. Rev 2 folds the architect review (accept-with-changes; all 15 findings adopted, dispositions in §7).
...
**3b — the tarball installs outside the workspace and its bins start on plain Node.** Pack with `pnpm pack`, not `npm pack`: only pnpm rewrites `workspace:8.0.0-rc.1` to `8.0.0-rc.1`, and `npm pack` would ship a specifier no registry consumer can resolve, failing the check for a reason that is an artifact of the tool. Install into a sandbox with `npm install`, which is the sanctioned exception and proves more than pnpm would: real users install with npm. Then run every entry in the packed manifest's `bin` map with plain `node` and require a zero exit.

Unpublished workspace siblings must be supplied locally. `@prisma/cli-engine@8.0.0-rc.1` is not on the registry — `latest` is `0.0.9` — so an unaided install of the shell's tarball fails on a missing version, the normal state of a lockstep release before its publish step. The sandbox maps each workspace sibling to its own packed tarball through npm `overrides`, with absolute `file:` paths (a relative path in a nested override resolves against `node_modules`, not the sandbox root) and **version-qualified keys** (`"@prisma/cli-engine@8.0.0-rc.1"`), for the reason 3c gives.

**3c — the shell's engine pin agrees with every family it mounts.** Compared between manifests: the packed manifest's `@prisma/cli-engine` version against the same field in each family package's manifest. The family manifest is read from the sandbox install of 3b, and 3c additionally asserts that the installed family version equals the version the shell's packed manifest declares, so a lockfile disagreeing with the declared pin is itself a finding.
...
- Fixtures live inside the repo. The one exception is 3b's sandbox, which must be outside any pnpm workspace or the workspace's own resolution defeats the install. composer already established the pattern for this (`check-npm-effect-resolution.mjs`), so it is precedent rather than a new exception.
...
**STOP-5 — take the stronger form of 3c as well as the manifest comparison?** Revision 1 asked you to confirm a proof that turned out to be wrong; §3 now records the correction. The install tree *can* carry the assertion, using a version-qualified override, and "exactly one engine copy resolves" catches a mismatch arriving through a family's own transitive dependency that a pairwise manifest comparison cannot see. It costs nothing extra given 3b installs anyway. **Recommended: take both** — the manifest comparison for its offline reach and its legible finding, the copy count for its strength. Say if you would rather have only the check as literally named in the mandate. (The out-of-workspace sandbox itself no longer needs a ruling: composer established the pattern, so §4 records it as precedent.)
