#!/usr/bin/env node
/**
 * Enumerate every published error code (dotted `NAMESPACE.SUBCODE`, see
 * docs/product/error-conventions.md) defined in production source, for the
 * error registry at docs/reference/error-reference.md and the hosted
 * reference at https://docs.prisma.io/docs/cli/error-reference.
 *
 * Modes:
 ...
 *                  lets the docs-site repo run it against a prisma-cli checkout.
 *
 * Codes are string literals whose namespace is on the closed list below.
...
const CODE_RE = new RegExp(
  `["']((?:${NAMESPACES.join("|")})\\.[A-Z][A-Z0-9_]*)["']`,
  "g",
);

const TEST_FILE_RE = /\.(test|test-d)\.ts$/;
