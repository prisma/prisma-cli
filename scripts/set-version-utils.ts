 ...
 *
 * Every scope is rewritten rather than one, because every lockstep package
 * in this workspace is versioned together — the published `@prisma/*`
 * packages and the private `@repo/*` packages alike. Singling out one scope
 * would leave the others pinned at whatever version they were last released
 * under.
 *
 ...
 * project-boundary manifests pin workspace packages and go stale on every
 * bump unless swept. Fixture manifests with only registry-style specs are
 * test data and must be left alone.
 */
export function participatesInLockstep(
  packageJson: MutablePackageJson,
): boolean {
  for (const field of DEP_FIELDS) {
    const deps = packageJson[field];
