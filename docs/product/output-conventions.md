...
- CI is detected
- a `prisma.config.ts` on the discovered config chain sets `skills: { check: false }` — a repository-root config silences it for every subdirectory that does not override the key
