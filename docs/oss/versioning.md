...
**History of the bare `prisma` name.** Its `latest` served Prisma 7 (`7.9.1`) until the 2026-08-25 cutover, so the v8 RC line lived on `next` while it matured. The products' packages were never held back — all new names with no version-7 audience — so they always published releases straight to `latest`, pre-release versions included, and this repository follows `latest` for them (operator ruling 2026-08-17).
...
- **`next`** — the same version as `latest`, kept in sync by the publish workflow's dist-tag step so existing `prisma@next` instructions keep resolving the newest release (operator ruling 2026-08-25). The sync needs the `NPM_DIST_TAG_TOKEN` repository secret — a granular npm token scoped to dist-tag maintenance only, an exception to the no-token rule precisely because OIDC cannot re-point tags on already-published versions; without the secret the step warns and the release completes with `next` unmoved. The tag keeps this meaning until it is deliberately given another.
