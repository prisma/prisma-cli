...
1. **Runs `pnpm bump-version`** on a fresh branch off `main`. The script reads the root version committed at HEAD, computes the next release version (`8.0.0-rc.N` → `8.0.0-rc.N+1`), and writes it to every lockstep `package.json`. Run `pnpm install` afterwards so the lockfile picks up the rewritten `workspace:` pins, then commit and open a `chore(release): 8.0.0-rc.N+1` PR.
