...
**When it expires** the automation stops opening pull requests and nothing fails loudly; the pins simply go stale. Record the renewal date somewhere visible. To replace it, generate a new token with the table above and update the secret in all three repositories; nothing in any repository names the account, so no code changes.
...
- **`@prisma/orm-toolchain`** — its released `8.0.0-rc.1` has no engine relationship and its `./cli` export contains no command family; the engine-based family exists only in dev builds, and the newest of those declares the engine at `0.0.9` rather than the version this repo ships. prisma/prisma needs to adapt to the current engine and publish that as a release.
