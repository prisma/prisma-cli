...
2. `packages/cli/src/main.ts` builds the CLI, runs the update check, hands the
   engine a runtime assembled from `process`, and after the command has run
   reports out-of-date agent skills.
...
- `.prisma/skills.json` stores whether the agent-skills staleness check is
  silenced for this project (written by `skills sync --disable`).
