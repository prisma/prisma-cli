...
export function buildFailedError(
  summary: string,
  cause: unknown,
): CliStructuredError {
  return new CliStructuredError("SERVICE.BUILD_FAILED", summary, {
    why: cause instanceof Error ? cause.message : String(cause),
    nextActions: [
      adviceAction(
        "Inspect the framework output, fix the build issue, and rerun the build.",
      ),
      runCommandAction("Rebuild", "service build"),
      runCommandAction("Deploy", "service deploy"),
    ],
    cause,
  });
}

export function frameworkNotDetectedError(cwd: string): CliStructuredError {
  return new CliStructuredError(
    "SERVICE.FRAMEWORK_NOT_DETECTED",
    `Cannot detect a supported framework in ${cwd}`,
    {
      why: "No supported Beta framework was found in the target directory.",
      nextActions: [
        adviceAction(
          "Add a supported framework as a dependency, pass --build-type, or pass --entry <path> for a Bun service.",
        ),
        runCommandAction(
...
export function buildSettingsUnsupportedError(
  ...
  return new CliStructuredError(
    "SERVICE.BUILD_SETTINGS_UNSUPPORTED",
    `build settings are not supported for ${displayName} services`,
    {
      why: `${displayName} deploys run \`${buildType} build\` and package its output automatically.`,
      nextActions: [
        adviceAction(
          "Remove the `build` block from prisma.compute.ts for this service.",
        ),
      ],
    },
  );
...
export function entrypointUnsupportedError(
  buildType: string,
): CliStructuredError {
  return new CliStructuredError(
    "SERVICE.ENTRYPOINT_UNSUPPORTED",
    `Service build does not accept --entry with --build-type ${buildType}`,
    {
      why: `${buildType} services do not use an entrypoint flag in the current preview.`,
      nextActions: [
        adviceAction(
          "Remove --entry, or rerun with --build-type bun when you want to target a Bun entrypoint directly.",
        ),
        runCommandAction(
          "Build a Bun service",
          "service build --build-type bun --entry server.ts",
...
export function buildDetectionAmbiguousError(
  supported: readonly string[],
): CliStructuredError {
  return new CliStructuredError(
    "SERVICE.BUILD_DETECTION_AMBIGUOUS",
    "Service build requires an explicit framework when detection is ambiguous",
    {
      why: `This preview auto-detects clear project shapes for ${supported.join(", ")}.`,
      nextActions: [
        adviceAction(
          "Pass a supported --build-type value, or pass --entry <path> for a Bun service.",
        ),
      ],
    },
  );
...
export function computeConfigInvalidError(options: {
  summary: string;
  why: string;
  where?: string;
  ...
  return new CliStructuredError(
    ...
    {
      why: options.why,
      ...(options.where ? { where: { path: options.where } } : {}),
      ...(options.meta ? { meta: options.meta } : {}),
      nextActions: [adviceAction(options.advice)],
    },
  );
}
