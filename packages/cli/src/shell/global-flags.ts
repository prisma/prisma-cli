...
export function addGlobalFlags(command: Command): Command {
  return command
    ...
    .addOption(
      new Option(
        "--interactive",
        "Force interactive behavior when prompts are supported.",
      ),
    )
    .addOption(
      new Option(
        "--no-interactive",
        "Disable interactive behavior and prompts.",
      ),
    )
    .addOption(
      new Option("--color", "Force color output in supported terminals."),
    )
...
export function resolveGlobalFlags(
  ...
  return {
    ...
    quiet:
      options.quiet === true || argv.includes("--quiet") || argv.includes("-q"),
    verbose:
