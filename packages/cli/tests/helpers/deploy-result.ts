 ...
 */
export function asSingleDeployResult<
  T extends { result: AppDeployResult | AppDeployAllResult },
>(success: T): T & { result: AppDeployResult } {
  if ("deployments" in success.result) {
    throw new Error(
      "Expected a single-app deploy result, got a deploy-all result.",
    );
  }
