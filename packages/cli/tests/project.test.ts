...
import { resolveActiveWorkspace } from "../src/commands/resources-shared/workspace";
...
async function tempCwd(pin?: { workspaceId: string; projectId: string }) {
  const cwd = await mkdtemp(path.join(os.tmpdir(), "project-test-"));
  if (pin) {
...
describe("prisma-cli project list", () => {
   ...
   *  was written — Prisma 7 wrote the API's `wksp_`-prefixed id, Prisma 8 writes the
