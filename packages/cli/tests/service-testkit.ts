...
import { mountedCommands } from "../src/cli";
  ...
  const dispatch = (method: string) => (path: string, init: unknown) => {
    ...
    if (!handler) {
      ...
      return Promise.reject(
        new Error(`service-testkit: unrouted request ${method} ${path}`),
      );
    }
 ...
 * test file restates a command path that `src/cli.ts` owns.
 */
...
export async function makeServiceCli(
  ...
  const cwd = await makeTempCwd("service-");
