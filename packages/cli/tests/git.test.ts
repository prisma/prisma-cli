...
import { gitConnectCommand } from "../src/commands/git/connect";
import { gitDisconnectCommand } from "../src/commands/git/disconnect";
...
async function pinnedCwd() {
  const cwd = await mkdtemp(path.join(os.tmpdir(), "git-test-"));
