...
describe("project rename", () => {
  it("renames the bound project and reports the previous name", async () => {
    const { cwd, stateDir } = await setupLinkedProject();

    const result = await executeCli({
      argv: ["project", "rename", "Acme Dashboard v2", "--json"],
      cwd,
      stateDir,
      fixturePath,
    });
    ...
    expect(payload).toMatchObject({
      ...
      result: {
        ...
        previousName: "Acme Dashboard",
      },
    });
  });

  it("renames an explicit --project target", async () => {
    const { cwd, stateDir } = await setupLinkedProject();
    ...
    expect(payload.result).toMatchObject({
      project: { id: "proj_456", name: "billing-core" },
      previousName: "Billing API",
    });
  });
