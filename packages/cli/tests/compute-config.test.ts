...
describe("normalizeComputeConfig", () => {
    ...
    const config = normalizeOrThrow(defineComputeConfig({
      apps: {
        ...
        worker: {
          ...
          framework: "bun",
          entry: "src/index.ts",
          env: { file: [".env", ".env.production"], vars: { LOG_LEVEL: "debug" } },
        },
      },
    }));
  ...
  it("rejects non-object default exports", () => {
    expect(normalizeIssues(undefined).join(" ")).toContain("export default defineComputeConfig");
    expect(normalizeIssues([1]).join(" ")).toContain("export default defineComputeConfig");
  });
  ...
  it("requires exactly one of app or apps", () => {
    ...
    expect(normalizeIssues({ apps: {} })).toEqual(["`apps` must define at least one app."]);
  });

  it("rejects unknown keys to catch typos", () => {
    expect(normalizeIssues({ app: {}, framework: "hono" }).join(" ")).toContain('Unknown top-level key "framework"');
    expect(normalizeIssues({ app: { htpPort: 8080 } }).join(" ")).toContain('Unknown key "htpPort"');
    expect(normalizeIssues({ app: { env: { files: ".env" } } }).join(" ")).toContain('Unknown key "files"');
  });
    ...
    const config = normalizeOrThrow(defineComputeConfig({
      apps: {
        web: {
          root: "apps/web",
          framework: "nextjs",
          build: { command: "pnpm --filter web build", outputDirectory: ".next/standalone/" },
        },
        api: {
          root: "apps/api",
          framework: "hono",
  ...
  it("validates build blocks", () => {
    ...
    expect(issues.join(" ")).toContain("`apps.web.build.command` must be a non-empty string, or null to skip the build step.");
    expect(issues.join(" ")).toContain('Unknown key "outputDir" in `apps.web.build`.');
    expect(issues.join(" ")).toContain('Unknown key "db" in `apps.web`.');
    expect(issues.join(" ")).toContain("`apps.api.build` must set `command` and/or `outputDirectory`.");
  });

  it("rejects entry with frameworks that derive entrypoints from build output", () => {
    const issues = normalizeIssues({ app: { framework: "nextjs", entry: "src/index.ts" } });
    expect(issues.join(" ")).toContain("`app.entry` is not supported with the nextjs framework");

    expect(normalizeOrThrow({ app: { framework: "hono", entry: "src/index.ts" } }).targets[0]?.entry)
      .toBe("src/index.ts");
  });
});
...
describe("selectComputeDeployTarget", () => {
  ...
  it("returns the single app without a target argument", () => {
    expect(selectComputeDeployTarget(single, undefined).unwrap().name).toBe("api");
  });
  ...
  it("rejects a target argument that does not match the single app", () => {
    ...
    expect(result.isErr() && result.error).toBeInstanceOf(ComputeConfigTargetUnknownError);
  });

  it("requires a target when multiple apps are configured", () => {
    const result = selectComputeDeployTarget(multi, undefined);
    expect(result.isErr() && result.error).toBeInstanceOf(ComputeConfigTargetRequiredError);
    if (result.isErr() && result.error instanceof ComputeConfigTargetRequiredError) {
      expect(result.error.availableTargets).toEqual(["web", "worker"]);
    }
  });

  it("selects a multi-app target by key", () => {
    expect(selectComputeDeployTarget(multi, "worker").unwrap().root).toBe("apps/worker");
  });

  it("defaults to the only app of a single-entry apps map", () => {
    const oneEntry = normalizeOrThrow({ apps: { web: { root: "apps/web" } } });
    expect(selectComputeDeployTarget(oneEntry, undefined).unwrap().key).toBe("web");
  });
...
describe("mergeComputeDeployInputs", () => {
  ...
  it("uses config values when flags are absent", () => {
    ...
    expect(merged.framework).toEqual({ value: "nextjs", annotation: "set by prisma.compute.ts" });
    expect(merged.httpPort).toEqual({ value: "8080", annotation: "set by prisma.compute.ts" });
    expect(merged.envInputs).toEqual([".env", "LOG_LEVEL=debug"]);
    expect(merged.configAppName).toEqual({ value: "web", annotation: "set by prisma.compute.ts" });
  ...
  it("prefers explicit flags over config values", () => {
    ...
    expect(merged.framework).toEqual({ value: "bun", annotation: "set by --framework" });
    expect(merged.entrypoint).toEqual({ value: "server.ts", annotation: "set by --entry" });
    expect(merged.httpPort).toEqual({ value: "3000", annotation: "set by --http-port" });
  ...
  it("passes CLI values through without a config file", () => {
    ...
    expect(merged.framework).toEqual({ value: "hono", annotation: "set by --framework" });
...
describe("mergeComputeLocalInputs", () => {
  ...
  it("maps the configured framework to a local build type", () => {
    ...
    expect(computeFrameworkToBuildType("tanstack-start")).toBe("tanstack-start");
  });
...
describe("loadComputeConfig", () => {
  ...
  it("loads a TypeScript config that imports @prisma/cli/config", async () => {
    ...
    expect(config?.kind).toBe("single");
    expect(config?.targets[0]).toMatchObject({ name: "api", framework: "hono", httpPort: 8080 });
  });

  it("loads a plain JavaScript config", async () => {
    const dir = await createTempDir();
    await mkdir(path.join(dir, "apps/web"), { recursive: true });
    await writeFile(path.join(dir, "prisma.compute.mjs"), [
      "export default {",
    ...
    const config = (await loadComputeConfig(dir)).unwrap();
    expect(config?.kind).toBe("multi");
    expect(config?.targets[0]).toMatchObject({ key: "web", root: "apps/web", framework: "nextjs" });
  });

  it("returns a load error for configs that fail to evaluate", async () => {
    const dir = await createTempDir();
    await writeFile(path.join(dir, "prisma.compute.ts"), "export default {", "utf8");

    const result = await loadComputeConfig(dir);
    expect(result.isErr() && result.error).toBeInstanceOf(ComputeConfigLoadError);
  });

  it("returns an invalid error for configs without a default export", async () => {
    const dir = await createTempDir();
    await writeFile(path.join(dir, "prisma.compute.ts"), 'export const app = { name: "api" };\n', "utf8");

    const result = await loadComputeConfig(dir);
    expect(result.isErr() && result.error).toBeInstanceOf(ComputeConfigInvalidError);
  });

  it("rejects multiple coexisting config files", async () => {
    const dir = await createTempDir();
    await writeFile(path.join(dir, "prisma.compute.ts"), "export default { app: {} };\n", "utf8");
    await writeFile(path.join(dir, "prisma.compute.js"), "export default { app: {} };\n", "utf8");

    const result = await loadComputeConfig(dir);
    expect(result.isErr() && result.error).toBeInstanceOf(ComputeConfigAmbiguousError);
  });
  ...
  it("discovers the config upward from a nested directory inside a repository", async () => {
    ...
    await writeFile(path.join(dir, "prisma.compute.ts"), 'export default { apps: { api: { root: "apps/api" } } };\n', "utf8");

    const config = (await loadComputeConfig(path.join(dir, "apps", "api", "src"))).unwrap();
  ...
  it("prefers the nearest config over an ancestor config", async () => {
    ...
    await writeFile(path.join(dir, "prisma.compute.ts"), 'export default { app: { name: "root" } };\n', "utf8");
    await writeFile(path.join(dir, "apps", "api", "prisma.compute.ts"), 'export default { app: { name: "nested" } };\n', "utf8");

    const config = (await loadComputeConfig(path.join(dir, "apps", "api"))).unwrap();
  ...
  it("does not search above the repository root", async () => {
    ...
    await writeFile(path.join(dir, "prisma.compute.ts"), 'export default { app: { name: "outside" } };\n', "utf8");

    expect((await loadComputeConfig(path.join(repo, "app"))).unwrap()).toBeNull();
  });

  it("does not walk upward without a repository boundary", async () => {
    const dir = await createTempDir();
    await mkdir(path.join(dir, "nested"), { recursive: true });
    await writeFile(path.join(dir, "prisma.compute.ts"), 'export default { app: { name: "parent" } };\n', "utf8");

    // No .git or workspace marker anywhere: only the invocation directory is checked.
    expect((await loadComputeConfig(path.join(dir, "nested"))).unwrap()).toBeNull();
  });

  it("observes config file edits across repeated loads", async () => {
    const dir = await createTempDir();
    const configPath = path.join(dir, "prisma.compute.ts");
    await writeFile(configPath, 'export default { app: { name: "one" } };\n', "utf8");
    expect((await loadComputeConfig(dir)).unwrap()?.targets[0]?.name).toBe("one");

    await writeFile(configPath, 'export default { app: { name: "two" } };\n', "utf8");
    expect((await loadComputeConfig(dir)).unwrap()?.targets[0]?.name).toBe("two");
  });
});
...
describe("compute config discovery and state location", () => {
  ...
  async function createTempDir(): Promise<string> {
    const dir = await mkdtemp(path.join(os.tmpdir(), "prisma-compute-discovery-"));
    tempDirs.push(dir);
    return dir;
  }
  ...
  it("anchors the default state directory at the config directory", async () => {
    ...
    const runtime = { cwd: appCwd, env: {} } as Parameters<typeof resolveStateDir>[0];
    expect(await resolveStateDir(runtime)).toBe(path.join(dir, ".prisma", "cli"));

    const standaloneRuntime = { cwd: dir, env: {}, stateDir: "/explicit/state" } as Parameters<typeof resolveStateDir>[0];
  ...
  it("keeps the default state directory at the invocation directory without a config", async () => {
    ...
    const runtime = { cwd: appCwd, env: {} } as Parameters<typeof resolveStateDir>[0];
    expect(await resolveStateDir(runtime)).toBe(path.join(appCwd, ".prisma", "cli"));
  });
});

describe("inferComputeTargetFromCwd", () => {
  function multiConfig(configDir: string, apps: Record<string, { root?: string }>): LoadedComputeConfig {
    const result = normalizeComputeConfig({ apps }, path.join(configDir, COMPUTE_CONFIG_FILENAME));
    if (result.isErr()) {
      throw result.error;
    }
    return result.value;
  ...
  it("infers the target whose root contains the invocation directory", () => {
    ...
    expect(inferComputeTargetFromCwd(config, "/repo/apps/api/src/routes")).toBe("api");
  ...
  it("infers nothing from the config directory or outside any root", () => {
    ...
    expect(inferComputeTargetFromCwd(config, "/repo/packages/db")).toBeUndefined();
  });
  ...
  it("infers nothing on an ambiguous tie", () => {
    ...
    expect(inferComputeTargetFromCwd(tied, "/repo/apps/shared")).toBeUndefined();
  });

  it("never infers for single-app configs", () => {
    const single = normalizeComputeConfig({ app: { name: "api" } }, "/repo/prisma.compute.ts");
    expect(inferComputeTargetFromCwd(single.unwrap(), "/repo/anywhere")).toBeUndefined();
  });
});

describe("computeConfigErrorToCliError", () => {
  it("maps config errors to structured CliErrors", () => {
    const invalid = computeConfigErrorToCliError(new ComputeConfigInvalidError(CONFIG_PATH, ["bad"]));
    expect(invalid).toBeInstanceOf(CliError);
    expect(invalid.code).toBe("COMPUTE_CONFIG_INVALID");
    expect(invalid.exitCode).toBe(2);

    const required = computeConfigErrorToCliError(new ComputeConfigTargetRequiredError(CONFIG_PATH, ["web", "worker"]));
    expect(required.code).toBe("COMPUTE_CONFIG_TARGET_REQUIRED");
    expect(required.nextSteps).toEqual(["prisma-cli app deploy web", "prisma-cli app deploy worker"]);

    const unknown = computeConfigErrorToCliError(new ComputeConfigTargetUnknownError(CONFIG_PATH, "docs", ["web"]));
