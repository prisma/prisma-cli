/**
 * The bin's wiring of the update check: the cached notification (and the
...
async function makeUpdateCheckDir(): Promise<string> {
  return await mkdtemp(path.join(os.tmpdir(), "update-check-"));
}
...
describe("main update-check wiring", () => {
  ...
  it("still notifies under the spelling --format json (literal-argv quirk, copied from legacy)", async () => {
    const updateCheckDir = await makeUpdateCheckDir();
    await seedStaleUpdate(updateCheckDir);
    ...
    // only; the spelling `--format json` is NOT suppressed. Recorded
