...
describe("isParentToSenderPayload", () => {
  ...
  it("accepts a payload with the optional databaseTarget override omitted entirely (no override)", () => {
    expect("databaseTarget" in validPayload).toBe(false);
    expect(isParentToSenderPayload(validPayload)).toBe(true);
  });
  ...
  it("rejects a payload whose databaseTarget override is the wrong type", () => {
    ...
    expect(
      isParentToSenderPayload({ ...validPayload, databaseTarget: null }),
    ).toBe(false);
  });
  ...
  it("rejects an empty endpoint", () => {
    expect(isParentToSenderPayload({ ...validPayload, endpoint: "" })).toBe(
      false,
    );
  });
  ...
  it("rejects flags when it is not a string array", () => {
    expect(
      isParentToSenderPayload({ ...validPayload, flags: "not-an-array" }),
    ).toBe(false);
    expect(isParentToSenderPayload({ ...validPayload, flags: [42] })).toBe(
      false,
    );
  });

  it("rejects a number where a string is expected", () => {
    expect(isParentToSenderPayload({ ...validPayload, version: 123 })).toBe(
      false,
    );
    expect(isParentToSenderPayload({ ...validPayload, projectRoot: 0 })).toBe(
      false,
    );
  });
});
